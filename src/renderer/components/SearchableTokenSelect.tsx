  ...
  return (
    <Popover.Trigger
      ...
      className={StyleHelper.mergeStyles(
        'bg-asphalt aria-expanded:bg-asphalt aria-[disabled=false]:hover:bg-asphalt/60 aria-[disabled=false]:focus:bg-asphalt/60 flex h-8.5 w-32 min-w-3 items-center gap-2 rounded px-4 aria-disabled:cursor-not-allowed aria-disabled:opacity-50',
        className
      )}
      disabled={disabled}
      {...props}
...
const List = ({ className, ...props }: TListProps) => {
  ...
  return (
    <Command.List ref={parentRef} className={StyleHelper.mergeStyles('mt-0.5 max-h-44', className)} {...props}>
      {isLoading ? (
          ...
          >
            {rowVirtualizer.getVirtualItems().map(virtualItem => {
              ...
              return (
                ...
                >
                  <div className="flex size-full min-w-0 items-center gap-2 text-sm text-white">
