...
export const ArticleListItem = ({ article, onSelect }: TProps) => {
  ...
  return (
    <li
      className="flex cursor-pointer items-center gap-x-2.5 rounded-sm px-2 py-3 transition-colors hover:bg-gray-300/15"
      onClick={onSelect}
      onKeyDown={handleKeyDown}
      tabIndex={0}
    >
      <div className="h-14 overflow-hidden rounded-sm">
        {article.previewImage ? (
          <img src={article.previewImage} alt={article.title} className="my-3 max-w-full rounded-lg" />
        ) : (
          <div className="bg-asphalt flex size-15 items-center justify-center rounded">
      ...
      <div>
        <TbChevronRight className="-mr-2 size-5 text-gray-300" aria-hidden />
      </div>
    </li>
  )
}
