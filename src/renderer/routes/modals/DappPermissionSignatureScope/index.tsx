...
export const DappPermissionSignatureScopeModal = () => {
  const { onReject, scope, allowedList, session } = useModalState<TModalState<'dapp-permission-signature-scope'>>()
      ...
      <div className="flex min-h-0 flex-grow flex-col overflow-y-auto pr-2 pl-5">
        ...
        {allowedList && (
          <Details.Root className="mt-3">
            ...
            <Details.Body>
              <p className="min-w-0 text-sm break-words text-gray-100">{allowedList?.join(',\r\n')}</p>
            </Details.Body>
          </Details.Root>
        )}
        ...
        <Details.Root className="mt-3">
          ...
          <Details.Body>
            <p className="min-w-0 text-sm break-words text-gray-100">{t(`scopes.${scope}`, t('scopes.unknown'))}</p>
          </Details.Body>
        </Details.Root>
      </div>
    </BottomModalLayout>
