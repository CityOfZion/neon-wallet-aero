...
export const Neo3DapiPermissionSignContent = ({ request, sendResult }: TDapiPermissionContentProps) => {
  const { network } = useSelectedNetworkSelector('neo3')

  const args = request.args as Parameters<DapiProvider['sign']>
  ...
  const [isAccepting, startAccept] = usePressOnce(async () => {
    try {
      ...
      const response = await operations.sign(...args)
      sendResult(response)
    } catch (error) {
