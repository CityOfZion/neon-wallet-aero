...
export const Neo3DapiPermissionSendContent = ({ request, sendResult }: TDapiPermissionContentProps) => {
  const { network } = useSelectedNetworkSelector('neo3')

  const args = request.args as Parameters<DapiProvider['send']>
  ...
  const [isAccepting, startAccept] = usePressOnce(async () => {
    try {
      ...
      const response = await operations.send(...args)
      sendResult(response)
    } catch (error) {
  ...
  const feeQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return await operations.calculateSendFee(...args)
    },
    gcTime: 0,
    staleTime: 0,
  })
