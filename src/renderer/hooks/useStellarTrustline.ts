...
export const useLazyStellarGetTrustlineTokens = () => {
  ...
  const getTrustlineTokens = async (filter: string) => {
...
export const usePersistTrustlineMutation = () => {
  ...
  return useMutation({
    ...
    mutationFn: async ({ stellarAccount, token, limit }: TUseStellarPersistTrustlineMutationParams) => {
      ...
      return { transaction, stellarAccount, token, limit }
    },
    onSuccess: async ({ transaction, token, limit, stellarAccount }) => {
      ...
      queryClient.setQueryData(
        buildStellarTrustlinesQueryKey(stellarAccount, selectedNetworkByBlockchain[stellarAccount.blockchain]),
        (oldData: TTrustlineServiceStellarGetTrustlinesResponse[]) => [...oldData, { token, limit: limit || '0' }]
      )
    },
    onError: error => {
