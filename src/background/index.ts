...
chrome.runtime.onMessage.addListener(async (message: TBackgroundMessage, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'get-login-session': {
      const response: TBackgroundGetLoginSessionResponse = { loginSession }
    ...
    case 'close-all-tabs':
      chrome.tabs.query({}, tabs => {
        for (const tab of tabs) {
          ...
          if (!!url && TabsHelper.isInTab(url) && typeof id === 'number') {
